...
class Trainer(object):
    ...
    def __init__(self, params, model: EnsembleGymEnv, agent: SAC_Agent, device):
        ...
        if self.transfer:
            self.reward_function.load_state_dict(torch.load('../../data/halfcheetah_test1_medexp_rank/model_saved_weights_seed1/torch_policy_weights_780_epochs_9869.pt'))
            # self.reward_function.load_state_dict(torch.load('./data/halfcheetah_test1_medexp_rank/model_saved_weights_seed2/torch_policy_weights_780_epochs_6920.pt'))
            # self.reward_function.load_state_dict(torch.load('./data/halfcheetah_test1_medexp_rank/model_saved_weights_seed3/torch_policy_weights_780_epochs_9869.pt'))
            # self.reward_function.load_state_dict(torch.load('./data/halfcheetah_test1_medexp_rank/model_saved_weights_seed4/torch_policy_weights_780_epochs_9869.pt'))
    ...
    def generate_score(self, mag, reward_bound=10.0):
        slope = -1
    ...
    def _train_reward(self):
        ...
        # ===============================
        intermediate_samples = np.arange(0.0,1.1,0.25) #np.arange(-0.4,1.4,0.2)
        traj_dataset = None
        label_dataset = None
        loss_fn = nn.MSELoss()

        for mag in intermediate_samples:
            idx = np.random.choice(expert_state_batch.shape[0],size=len(agent_state_batch))
            sM = agent_state_batch + mag*(expert_state_batch[idx]-agent_state_batch) # trajectory mixup
            sMa = agent_action_batch + mag*(expert_action_batch[idx]-agent_action_batch) # trajectory mixup
            # sM_vec = sM
            sM_vec = torch.cat((sM, sMa), 1)
            val = self.generate_score(mag, reward_bound=10)
            if traj_dataset is None:
                traj_dataset = sM_vec
                label_dataset = torch.ones((sM_vec.shape[0],sM_vec.shape[1]))*int(val)
            else:
                traj_dataset = torch.cat((traj_dataset,sM_vec),dim=0)
                labels = torch.ones((sM_vec.shape[0],sM_vec.shape[1]))*int(val)
                label_dataset = torch.cat((label_dataset,labels),dim=0)
                
        label_dataset = label_dataset.to(self._device)
        idx = np.arange(traj_dataset.shape[0])
        holdout_size = int(0.1*(traj_dataset.shape[0]))
        train_dataset = traj_dataset[idx[holdout_size:],:]
        train_label = label_dataset[idx[holdout_size:]]

        holdout_dataset = traj_dataset[idx[:holdout_size],:]
        holdout_label = label_dataset[idx[:holdout_size]]
        ...
        for i in range(0,idx.shape[0],batch_size):
            ...
            label_x = train_label[idx[i:min(i+batch_size,idx.shape[0])]]
            t1 = self.reward_function(train_x)
            loss = loss_fn(t1, label_x.to(self._device))
            ...
            for i in range(0, holdout_dataset.shape[0],batch_size):
                ...
                holdout_label_x = holdout_label[i:min(i+batch_size,holdout_dataset.shape[0])]
                t1 = self.reward_function(holdout_x)
                loss_val = loss_fn(t1, holdout_label_x.to(self._device))
        ...
        #  
        # t1 = self.reward_function(traj_dataset)
        # rankLoss = 1e-3 * loss_fn(t1, label_dataset.to(self._device))
        # print('rank loss', rankLoss)
        # ===============================
        
        #regularizer = avg_reward_norm
        # loss = agent_r.mean() - expert_r.mean()
        # print('ml loss', mlLoss)
        # loss_criterion = nn.CrossEntropyLoss()
        # print('regular loss', loss)
        # anotherloss = loss_criterion(torch.cat((torch.mean(agent_r).unsqueeze(0), torch.mean(expert_r).unsqueeze(0))), torch.from_numpy(np.array(1)).to(self._device))
        # print('pref loss', anotherloss)
        # loss += anotherloss
        # loss = loss + rankLoss
        # loss = loss_criterion(agent_r, expert_r)
        # loss = loss_criterion(torch.cat((r_i.unsqueeze(0), r_j.unsqueeze(0))), label)
    ...
    def train_offline(self, num_epochs, save_model=False, save_policy=False, load_model_dir=None):
        ...
        for i in range(num_epochs):
            ...
            if i % 40 == 0 and not self.transfer: # update every 40
                self._train_reward()
            
            reward_model = self.test_agent(use_model=True, n_evals=5)
            ...
            if hasattr(self, "population_models"):
                reward_pop = self.test_agent_population(full_trajectories=True, n_evals=2)
